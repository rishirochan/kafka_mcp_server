...
async def server_lifespan(server: FastMCP) -> AsyncIterator[AppContext]:
    ...
    try:
        kafka_connector = KafkaConnector(bootstrap_servers=bootstrap_servers, schema_registry_url=schema_registry_url)
...
@mcp_server.tool("publish", description="Use this tool when you need to publish a message to a topic. If a Schema Registry is configured and a schema exists for the topic, the value will be Avro-encoded automatically.")
async def publish(ctx: Context[ServerSession, AppContext], topic_name: str, value: str, key: Optional[str] = None, session_id: Optional[str] = None, schema_type: Optional[str] = None):
    ...
    """
    kafka_connector = ctx.request_context.lifespan_context.kafka_connector
    return await kafka_connector.publish(topic_name, value, key, session_id, schema_type)
...
@mcp_server.tool("list_schemas", description="Use this tool to list all schema subjects registered in the Schema Registry")
def list_schemas(ctx: Context[ServerSession, AppContext]):
    ...
    if kafka_connector.schema_registry is None:
        return {"error": "Schema Registry is not configured. Set SCHEMA_REGISTRY_URL environment variable."}
    return kafka_connector.schema_registry.get_subjects()


@mcp_server.tool("get_schema", description="Use this tool to get a schema by subject name and version from the Schema Registry")
def get_schema(ctx: Context[ServerSession, AppContext], subject: str, version: Optional[str] = "latest"):
...
@mcp_server.tool("register_schema", description="Use this tool to register a new Avro schema for a subject in the Schema Registry")
def register_schema(ctx: Context[ServerSession, AppContext], subject: str, schema_str: str, schema_type: str = "AVRO"):
...
@mcp_server.tool("delete_schema", description="Use this tool to delete all versions of a schema subject from the Schema Registry")

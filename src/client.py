...
class KafkaClient:
    ...
    async def kafka_agent(self):
        ...
        async with self.client.session("kafka") as session:
            ...
            print("kafka_publish_status ", self.get_messages(kafka_publish_status, "tool"))
            ...
            kafka_consume_status = await agent.ainvoke(
                get_consume_message(
                    session_id="consumer_01", 
                    topic_name="my_topic1"
                )
            )
            print("kafka_consume_status ", self.get_messages(kafka_consume_status, "tool"))
